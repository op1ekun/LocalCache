...
(function($){
    ...
    var STORAGE_LIMIT   = 5 * 1024 * 1024; //  5 242 880 bytes;
    
    // custom Error
    function NotEnoughSpaceError() {};
    NotEnoughSpaceError.prototype = new Error();
    ...
    function add(key, item, doNotOverwrite) {
        ...
        if (stringified.length > availableSpace) {
            if (stringified.length > STORAGE_LIMIT) {
                throw new RangeError("item size exceeds capacity of the Storage");                
            }

            throw new NotEnoughSpaceError("not enough space left in the Storage");
        }
        else {
            window[TYPE].setItem(
                key,
